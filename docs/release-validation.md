...
| Claude Desktop end to end | Registered from the ribbon on the Store build: written to the package LocalCache path, `.arcgis-mcp.bak` matched the original, `coworkUserFilesPath` and `preferences` kept. The app then called two tools through the generated PowerShell bridge |
| Claude Desktop install layouts | The Store build on this machine resolves to `%LOCALAPPDATA%\Packages\Claude_pzs8sxrjxfjjc\LocalCache\Roaming\Claude\claude_desktop_config.json` and now reports as installed; the classic `%APPDATA%\Claude` layout is still selected when no package is present. Registering from the ribbon and restarting the app is untested -- the real config was left alone |
